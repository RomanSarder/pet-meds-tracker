...
async function nextDueFromRepo(repo: Repo, course: Course, after: Date): Promise<Date | null> {
  ...
  // The repo's own ledger, unbounded — the real thing `HistoryPage.tsx`
  // threads into the sheet, not an empty stand-in: `createCourse` writes a
  // "started" CourseEvent, so `[]` here would already be a lie.
  const courseEvents = await repo.listCourseEvents({ courseId: course.id });
  return nextDueAt(course, events, courseEvents, after);
}
