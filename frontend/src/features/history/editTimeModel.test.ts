...
describe("consequenceFor", () => {
  const now = at(DAY, "18:00");
  // None of this describe block's fixture courses go through a ledger — every
  // `Course` here is a plain object literal, never run through `createCourse`
  // or `updateCourse`, so it has genuinely never been edited. `[]` is the
  // correct ledger for all of them (see `consequenceFor`'s doc on why an
  // empty ledger is wrong for a course that WAS edited).
  const noCourseEvents: CourseEvent[] = [];
  ...
  it("moves nothing when a fromLastDose dose that is NOT the last one is edited", () => {
    ...
    expect(
      consequenceFor({ course, events, event: first, chosen: at(DAY, "09:00"), courseEvents: noCourseEvents }),
    ).toEqual({ kind: "unchanged" });
    ...
    expect(
      nextDueAt(course, [...events, corrected], noCourseEvents, at(DAY, "05:00")),
    ).toEqual(at(NEXT_DAY, "00:00"));
  });
  ...
  it("previews exactly what the engine will compute after the write", () => {
    ...
    const actual = nextDueAt(course, [first, last, written], noCourseEvents, chosen);
