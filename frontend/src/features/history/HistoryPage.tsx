...
export function HistoryView({ petId }: { petId: string }) {
  ...
  // The sheet's consequence preview: EVERY CourseEvent of the one course
  // being edited, unbounded — like `editEvents` above, but for the ledger
  // `consequenceFor` hands to `nextDueAt` (SPEC §3c reconstructs the
  // schedule in effect at each slot's own due instant from it). The
  // `courseEvents` fetched above is bounded to the visible `from`/`to`
  // window, which is fine for the rendered log but wrong as engine input —
  // an edit made before the window would be invisible to it, silently
  // reproducing the retroactive-rewrite bug this branch fixes. Mirrors
  // `PetDetailPage.tsx`'s split between its bounded `courseEventsForRecent`
  // (display) and its unbounded `courseEvents` (engine input).
  const editCourseEvents = useCourseEventLog(
    { courseId: editing?.courseId },
    { enabled: editing !== null },
  );
  ...
  // Not defaulted to `[]` when absent — see `editCourseEvents`'s own comment
  // above. Held in a local so the render guard below can narrow it to a real
  // array rather than the sheet ever receiving a placeholder ledger.
  const editCourseEventsData = editCourseEvents.data;
  ...
  return (
    <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
        ...
        <Menu.Portal>
          <Menu.Positioner sideOffset={4} align="end">
            <Menu.Popup
              ...
              // `var(--surface)` and `var(--line-quiet)` in MENU_POPUP_STYLE
      ...
      */}
      {editing !== null && editingEntry && editingCourse && editingEvent && editCourseEventsData ? (
        <EditDoseTimeSheet
          open
          onOpenChange={(next) => {
          ...
          courseEvents={editCourseEventsData}
...
function RowMenu({ entry, onEditTime }: { entry: LogEntry; onEditTime: () => void }): ReactElement {
  ...
  return (
    <Menu.Root>
      ...
      <Menu.Portal>
        <Menu.Positioner sideOffset={4} align="end">
          {/* `className="ds-root"` for the same reason as the export menu above:
              a portalled popup lands outside the `.ds-root` wrapper, so without
              it every token in MENU_POPUP_STYLE resolves to nothing. */}
