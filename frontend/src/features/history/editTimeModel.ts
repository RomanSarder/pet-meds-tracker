 ...
 *                 generated from the calendar and the CourseEvent ledger, so
 *                 no logged time can move it), and any `fromLastDose` dose
 *                 that is not the chain's anchor.
...
export function consequenceFor(args: {
  ...
  chosen: Date;
  /**
   * The course's real CourseEvent ledger — REQUIRED, and it must be the
   * unbounded one. `nextDueAt` reconstructs the schedule in effect at each
   * slot's own due instant from this ledger (SPEC §3c); a caller that passed
   * `[]` for a course that has actually been edited would make the engine
   * fall back to the live schedule, silently reproducing the retroactive
   * bug this file's header describes.
   */
  courseEvents: CourseEvent[];
}): { kind: "unchanged" } | { kind: "moves"; next: Date; deltaMin: number } {
  const { course, events, event, chosen, courseEvents } = args;

  // Answered structurally rather than by diffing, but NOT for the reason the
  // old comment gave. The fixedTimes grid is no longer "a function of the
  // calendar alone" — `scheduleTimelineFor` (engine/occurrences.ts) makes it
  // a function of the calendar AND of which schedule version the ledger says
  // governed each slot. What still makes this early return safe is narrower:
  // correcting a dose's `givenAt` only ever changes `events`, never
  // `courseEvents`, and a fixedTimes occurrence's `dueAt` (the only field
  // `nextDueAt` reads) is computed from `course` + `courseEvents` alone —
  // `buildFixedTimesOccurrence` never consults `events` for the due instant,
  // only for the `event` attached to it, which `nextDueAt`'s loop never
  // looks at. So `before` and `after` would call `nextDueAt` with identical
  // `course` and identical `courseEvents`, differing only in `events`, which
  // cannot move a fixedTimes `dueAt` — `nextDueAt` is provably equal either
  // side of this write, without needing to call it twice to find out.
  ...
  const before = nextDueAt(course, events, courseEvents, reference);
  const after = nextDueAt(course, [...events, correctedEvent(event, chosen)], courseEvents, reference);
