...
export function shoppingListText(items: SupplyItem[], tr: Translator): string {
  return items
    .map((item) => {
      // The pack branch renders "N more pack(s)"; the no-packSize branch
      // renders a unit-bearing quantity (e.g. "3 ml").
      const quantity = neededLabel(item.projection.neededPacks, item.projection.needed, item.unit, tr);
